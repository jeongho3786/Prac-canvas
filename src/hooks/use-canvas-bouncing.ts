...
interface UseCanvasBouncingParams {
  canvasContext: CanvasRenderingContext2D | null;
  canvasSize: CanvasSize;
}

const useCanvasBouncing = ({
  canvasContext,
  canvasSize,
}: UseCanvasBouncingParams) => {
  useEffect(() => {
    ...
    const boxMaxX = boxPositionX + boxWidth;
    const boxMaxY = boxPositionY + boxHeight;
    ...
    const bounceBlock = () => {
      ...
      const maxContactX = boxMaxX + radius;
      const minContactY = boxPositionY - radius;
      const maxContactY = boxMaxY + radius;
    ...
    const draw = () => {
      ...
      canvasContext.moveTo(boxMaxX, boxMaxY);
      canvasContext.lineTo(boxMaxX - shadowPointX, boxMaxY + shadowPointY);
      canvasContext.lineTo(boxPositionX - shadowPointX, boxMaxY + shadowPointY);
      canvasContext.lineTo(boxPositionX, boxMaxY);
      canvasContext.fill();

      // 장애물 사이드
      canvasContext.fillStyle = "#9d0919";
      canvasContext.beginPath();
      canvasContext.moveTo(boxPositionX - shadowPointX, boxMaxY + shadowPointY);
...
export default useCanvasBouncing;
